#!/usr/bin/env python3
"""
Datagateway — Football Fetcher
Ambil data pertandingan + odds dari TheRundown API (FIFA World Cup 2026).
SQLite cache 10 menit.
"""
...
if not API_KEY:
    # Fallback to a file that's gitignored
...
# Sports that are football/soccer — only check leagues likely in season
# European leagues: Aug-May, MLS: Mar-Oct, FIFA World Cup: Jun-Jul 2026
FOOTBALL_SPORTS = [
    (18, "FIFA World Cup"),
    (10, "MLS"),
    # European leagues are offseason (Jun-Aug), skip until August
]
# Check extra leagues only on weekday basis to save API quota
EXTRA_SPORTS = [
    (11, "EPL"),
    (14, "La Liga"),
    (15, "Serie A"),
...
def fetch_events(sport_id: int, sport_name: str, date: str) -> list[dict]:
    ...
    try:
        resp = requests.get(url, headers={"X-TheRundown-Key": API_KEY, "User-Agent": USER_AGENT}, timeout=20)
...
def fetch_event_odds(event_id: str) -> list[dict]:
    ...
    try:
        resp = requests.get(url, headers={"X-TheRundown-Key": API_KEY, "User-Agent": USER_AGENT}, timeout=15)
        resp.raise_for_status()
        data = resp.json()
        events = data.get("events", [])
        odds_data = []
                ...
                for p in m.get("participants", []):
                    for l in p.get("lines", []):
                        ...
                        for aid_str, price_data in l.get("prices", {}).items():
                            # Get line_value from the line itself
                            odds_data.append({
                                "event_id": event_id,
...
def main():
    ...
    for sport_id, sport_name in FOOTBALL_SPORTS:
        for date in [today, tomorrow]:
            ...
            # Fetch odds for ALL events (up to 6)
            # Sort so scheduled matches get priority
            ...
            for ev in sorted_events:
                ...
                odds_count = sum(1 for o in odds_data if isinstance(o, dict) and football_upsert_odds(o))
                if odds_count:
